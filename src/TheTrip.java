/*
A group of students are members of a club that travels annually to diﬀerent locations.
...
public class TheTrip {
    /*
        ...
        static double[] studentSpent = {totalStudents};
        static double total_high = 0.0;
        static double total_low = 0.0;
        ...
        static double sum (int n){
            double result = 0;
            for (int i = 0; i < n; i++){
                result += studentSpent[i];
            }
            return result;
        }

        private static void problem() throws IOException {
            Path file = Path.of("resources/trip");
            List<String> lines = Files.readAllLines(file);
            //System.out.println(Arrays.toString(studentSpent));
